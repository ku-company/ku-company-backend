...
describe('JobPostingService', () => {
	...
	it('get_all_job_postings transforms jobs to feed DTO and calls get_profile_image', async () => {
		...
		const jobs = [
			{
				...
				company: { company_name: 'Acme', location: 'BKK', tel: '123', user_id: 10 },
			...
			},
			{
				...
				company: { company_name: 'Globex', location: 'CNX', tel: '555', user_id: 11 },
				created_at: new Date(now.getTime()), // today
				updated_at: new Date(now.getTime()),
			},
		];

		(svc as any).jobPostingRepository.get_all_job_postings.mockResolvedValueOnce(jobs);
		...
		expect((svc as any).jobPostingRepository.get_all_job_postings).toHaveBeenCalledWith(
			'kw',
			'Developer',
			'Internship'
		);
	...
	it('get_job_posting_by_id returns null when not found', async () => {
		const svc = makeSvc();
		(svc as any).jobPostingRepository.get_job_posting_by_id.mockResolvedValueOnce(null);
	...
	it('get_job_posting_by_id transforms item and calls get_profile_image', async () => {
		...
		const item = {
			...
			company: { company_name: 'Initech', location: 'BKK', tel: '777', user_id: 20 },
			created_at: new Date(now.getTime() - 3 * 86400000), // 3 days ago
			updated_at: new Date(now.getTime()),
		} as any;
		(svc as any).jobPostingRepository.get_job_posting_by_id.mockResolvedValueOnce(item);
