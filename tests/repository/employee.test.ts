...
describe('EmployeeRepository - profile CRUD', () => {
  ...
  it('create_profile creates and converts birthDate', async () => {
    ...
    const req: any = { user: { id: 55 }, body: { first_name: 'A', birthDate: '2000-01-02' } };
  ...
  it('edit_profile updates and converts birthDate', async () => {
    ...
    const out = await repo.edit_profile(12, { first_name: 'B', birthDate: '1999-12-31' } as any);
    expect(out).toBe(sample);
  ...
  it('edit_profile wraps thrown errors', async () => {
    ...
    await expect(repo.edit_profile(1, { first_name: 'X' } as any)).rejects.toThrow(
      'Failure becauseboom'
    );
  });
});
...
describe('EmployeeRepository - resumes', () => {
  ...
  it('get_resumes, resume_count call prisma correctly', async () => {
    ...
    expect(cnt).toBe(5);
    expect(mockPrisma.resume.findMany).toHaveBeenCalledWith({ where: { employee_id: 22 } });
    expect(mockPrisma.resume.count).toHaveBeenCalledWith({ where: { employee_id: 22 } });
  });
  ...
  it('get/delete resume by id use composite where', async () => {
    ...
    expect(mockPrisma.resume.findUnique).toHaveBeenCalledWith({ where: { id: 2, employee_id: 33 } });
    await repo.delete_resume_by_id(2, 33);
    expect(mockPrisma.resume.delete).toHaveBeenCalledWith({ where: { id: 2, employee_id: 33 } });
  });
  ...
  it('delete_resumes_by_profile_id calls deleteMany', async () => {
    ...
    expect(mockPrisma.resume.deleteMany).toHaveBeenCalledWith({ where: { employee_id: 44 } });
  });
  ...
  it('find_main_resume, set/unset_main_resume shape', async () => {
    ...
    expect(mockPrisma.resume.findFirst).toHaveBeenCalledWith({ where: { employee_id: 55, is_main: true } });
...
describe('EmployeeRepository - apply_to_individual_job', () => {
  it('throws when user already has a job', async () => {
    const repo = makeRepo();
    mockPrisma.employeeProfile.findUnique.mockResolvedValue({ id: 1, has_job: true });
  ...
  it('throws when already applied', async () => {
    const repo = makeRepo();
    mockPrisma.employeeProfile.findUnique.mockResolvedValue({ id: 2, has_job: false });
  ...
  it('throws when job not found or invalid status or no slots', async () => {
    ...
    // invalid status
    mockPrisma.jobPost.findUnique.mockResolvedValueOnce({ id: 1, status: 'Closed', available_position: 1 });
    await expect(repo.apply_to_individual_job(1, 10, 5)).rejects.toThrow(
      'Failed to apply to job: This job cannot be applied to'
    );

    // no slots
    mockPrisma.jobPost.findUnique.mockResolvedValueOnce({ id: 1, status: 'Active', available_position: 0 });
    await expect(repo.apply_to_individual_job(1, 10, 5)).rejects.toThrow(
  ...
  it('throws when resume missing or not found', async () => {
    ...
    mockPrisma.employeeProfile.findUnique.mockResolvedValue({ id: 2, has_job: false });
    mockPrisma.jobApplication.findFirst.mockResolvedValue(null);
    mockPrisma.jobPost.findUnique.mockResolvedValue({ id: 1, status: 'Active', available_position: 1 });
  ...
  it('creates application when all checks pass', async () => {
    ...
    mockPrisma.employeeProfile.findUnique.mockResolvedValue({ id: 2, has_job: false });
    mockPrisma.jobApplication.findFirst.mockResolvedValue(null);
    mockPrisma.jobPost.findUnique.mockResolvedValue({ id: 1, status: 'Active', available_position: 3 });
...
describe('EmployeeRepository - list/cancel applications', () => {
  it('list_own_resume errors when no profile or no resumes', async () => {
    ...
    await expect(repo.list_own_resume(1)).rejects.toThrow('Employee profile not found');
  ...
  it('cancel_application errors when owner or application missing, else deletes', async () => {
    const repo = makeRepo();
    mockPrisma.employeeProfile.findUnique.mockResolvedValueOnce(null);
    await expect(repo.cancel_application(1, 2)).rejects.toThrow('Employee profile not found');
    ...
    await expect(repo.cancel_application(1, 2)).rejects.toThrow('Application not found');
  ...
  it('list_all_applications errors when empty, else returns list', async () => {
    ...
    await expect(repo.list_all_applications(1)).rejects.toThrow('No applications found');
...
describe('EmployeeRepository - batch apply and confirmation', () => {
  it('apply_job_checkout_list fails when profile missing or batch create fails or already applied', async () => {
    const repo = makeRepo();
    mockPrisma.employeeProfile.findUnique.mockResolvedValueOnce(null);
    await expect(repo.apply_job_checkout_list(1, 5, [10, 11])).rejects.toThrow('Employee profile not found');
    ...
    await expect(repo.apply_job_checkout_list(1, 5, [10, 11])).rejects.toThrow('Failed to create batch');
  ...
  it('apply_job_checkout_list calls apply_to_individual_job for each id', async () => {
    ...
    mockPrisma.jobApplication.findFirst.mockResolvedValue(null);
    const spy = jest.spyOn(repo as any, 'apply_to_individual_job').mockResolvedValue('ok' as any);
  ...
  it('sent_the_confirmation_to_company: errors on not confirmed and already has job', async () => {
    ...
    mockPrisma.jobApplication.findUnique.mockResolvedValueOnce(null);
    await expect(repo.sent_the_confirmation_to_company(1, 2)).rejects.toThrow('Job application not found');

    // company not confirmed
    mockPrisma.employeeProfile.findUnique.mockResolvedValueOnce({ id: 2, has_job: false });
    mockPrisma.jobApplication.findUnique.mockResolvedValueOnce({ id: 3, company_send_status: 'Pending', job_post: { company_id: 9 } });
    await expect(repo.sent_the_confirmation_to_company(1, 2)).rejects.toThrow(
      'This application, Company has not confirmed yet'
    );

    // already has job
    mockPrisma.employeeProfile.findUnique.mockResolvedValueOnce({ id: 2, has_job: true });
    mockPrisma.jobApplication.findUnique.mockResolvedValueOnce({ id: 3, company_send_status: 'Confirmed', job_post: { company_id: 9 } });
    await expect(repo.sent_the_confirmation_to_company(1, 2)).rejects.toThrow(
  ...
  it('sent_the_confirmation_to_company: success flow returns notification and updates related entities', async () => {
    ...
    mockPrisma.jobApplication.findMany.mockResolvedValue([{ id: 30, job_post: { company_id: 1 } }]);
    ...
    expect(mockPrisma.jobApplication.update).toHaveBeenCalledWith({
      where: { id: 2 },
      data: { employee_send_status: 'Confirmed', employee_responded_at: expect.any(Date) },
    });
