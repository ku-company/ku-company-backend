...
describeIf('Integration: AnnouncementRepository', () => {
  ...
  it('get_post_by_id and get_all_posts include job_post and order desc', async () => {
    ...
    const a1 = await prisma.announcement.create({ data: { professor_id: prof.id, type_post: 'Opinion', content: 'Op' } });
    const a2 = await prisma.announcement.create({ data: { professor_id: prof.id, type_post: 'Repost', job_id: job.id, is_connection: true } });
