...
export class EmployeeController{
    ...
    async sent_the_confirmation_to_company(req: any, res: any){
        ...
        try{
            ...
            const result = await this.employeeService.add_comment_to_company(req.user.id, company_id, req.body.comment)
            res.status(200).json({
    ...
    async edit_comment(req: any, res: any){
        try{
            ...
            const result = await this.employeeService.edit_comment(req.user.id, comment_id, req.body.comment)
            res.status(200).json({
    ...
    async delete_comment(req: any, res: any){
        try{
            const comment_id = Number(req.params.id)
            const result = await this.employeeService.delete_comment(req.user.id, comment_id)
            res.status(200).json({
